...
class LangTranslate extends Controller{
    ...
    public function lst() {
        ...
        $list = $translateModel->getLangKeyList($keyword, 30)
    ...
    public function add() {
        if (request()->isPost()) {
            $data = input('post.');
            $result = $this->validate($data, '\addons\lang\validate\LangTranslate');
            if(true !== $result){
    ...
    public function edit() {
        ...
        if (request()->isPost()) {
            ...
            Db::startTrans();
            try{
                $translateModel->updateLangKey($data['id'],['key_name'=>trim(($data['key_name'])),'remark'=>trim($data['remark'])]);
...
//                    $this->writeLang($data['key_name'], $data['value_name'][$key], $data['lang_code'][$key]);
                ...
                return json(array('status' => 0, 'mess' => lang('编辑失败')));
            }
            return json(array('status' => 1,'mess' => lang('编辑成功')));
        }
        $id = input('id');
    ...
    public function createLangPackage() {
        ...
        foreach ($key_list as $item) {
            ...
            foreach ($value_list as $v) {
                $lang[$v['lang_code']][trim($item['key_name'])] = trim($v['value_name']);
            }
        }
        $langList = $translateModel->getLangList();
        foreach ($langList as $v) {
            $lang_file = APP_PATH  . "/lang/{$v['lang_code']}.php";
...
//	                }
            ...
            if (!empty($lang_data)) {
                $lang_file = APP_PATH  . "/lang/{$lang_code}.php";
