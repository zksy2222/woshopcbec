...
layui.define(['layer', 'laytpl', 'upload'], function(exports){
  ...
  layui.data.content = function(content){
    ...
    if(content.substr(0,5) == 'goods'){ // 处理商品消息
      let str = content.replace(/(^goods\[)|(\]$)/g, '');
      let str2 = str.replace(/&quot;/g, '"');
      const goodsObj = JSON.parse(str2);
      return `<div style="display:flex;"><div style="margin-right: 10px;"><img src="${goodsObj.goodsThumb}" style="width:60px;height:60px;" /></div><div><h4>${goodsObj.goodsName}</h4><p style="margin-top:10px;">¥${goodsObj.shopPrice}</p></div></div>`;
    }
    content = (content||'').replace(/&(?!#?[a-zA-Z0-9]+;)/g, '&amp;')
        ...
        .replace(/img\[([^\s]+?)\]/g, function(img){  //转义图片
          return '<img class="layui-layim-photos" style="max-width: 100%;" src="' + img.replace(/(^img\[)|(\]$)/g, '') + '">';
        })
      ...
      layui.upload({
        ...
        ,success: function(res){
          ...
          if(res.status == 200){
            res.data = res.data || {};
            if(type === 'images'){
              focusInsert(thatChat.textarea[0], 'img['+ (res.data.full_path||'') +']');
            } else if(type === 'file'){
              focusInsert(thatChat.textarea[0], 'file('+ (res.data.full_path||'') +')['+ (res.data.name||'下载文件') +']');
            }
          } else {
            layer.msg(res.mess||'上传失败');
          }
        }
      });
    }
