...
class Version extends Common{
    ...
    public function lst(){
        ...
        $list = Db::name('version')->order('create_time DESC')->select();
        foreach($list as $k=>$v){
...
//            }
    ...
    public function edit(){
        if(request()->isPost()) {
            ...
            $versionInfos = Db::name('version')->where('id', $data['id'])->field('id')->find();
            if (!$versionInfos) {
                datamsg(0, '编辑失败');
            }
            $count = Db::name('version')->update($data, array('id' => $data['id']));
            if ($count === false) {
                datamsg(0, '编辑失败');
            }
            ys_admin_logs('编辑app版本控制', 'version', $data['id']);
            datamsg(1, '编辑成功');
        }else {
            $id = input('id');
            if(!$id){
            ...
            $version = Db::name('version')->find($id);
            if(!$version){
                datamsg(0,'找不到相关信息');
            }
